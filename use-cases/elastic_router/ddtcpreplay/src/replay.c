...
extern char loop_state;
extern int run_loop;
extern pthread_mutex_t dd_startm;
extern pthread_cond_t dd_runc;

// start tcpreplay with the files in the correct order!
// rise, high, fall, low
// tcpreplay -K -i dummy0 -l 0 /run/tdata/rise-er1.pcap /run/tdata/high-er1.pcap
// /run/tdata/fall-er1.pcap /run/tdata/low-er1.pcap
char f_index = 0;
...
int tcpr_replay_index(tcpreplay_t *ctx) {
  ...
  if (loop_state == ST_LOW) {
    f_index = ST_LOW;
  } else if (loop_state == ST_HIGH) {
    f_index = ST_HIGH;
  } else if (loop_state == ST_RISE) {
    f_index = ST_RISE;
  } else if (loop_state == ST_FALL) {
    f_index = ST_FALL;
  } else {
    fprintf(stderr, "Unknown loop state %d!\n", loop_state);
  }
  ...
  if (!ctx->options->dualfile) {
    ...
    for (idx = 0; idx < ctx->options->source_cnt && !ctx->abort; idx++) {
      ...
      case source_cache:
        ...
        break;
      default:
        tcpreplay_seterr(ctx, "Invalid source type: %d",
                         ctx->options->sources[idx].type);
        rcode = -1;
      }
    }
  }
  ...
  /* dual file mode: two files, two interfaces */
    ...
    for (idx = 0; idx < ctx->options->source_cnt && !ctx->abort; idx += 2) {
      if (ctx->options->sources[idx].type !=
          ctx->options->sources[(idx + 1)].type) {
        tcpreplay_seterr(
            ctx, "Both source indexes (%d, %d) must be of the same type", idx,
            (idx + 1));
        return -1;
      }
      switch (ctx->options->sources[idx].type) {
  ...
  if (loop_state == ST_LOW && f_index == ST_LOW) {
    f_index = ST_LOW;
  } else if (loop_state == ST_HIGH && f_index == ST_HIGH) {
    f_index = ST_HIGH;
  } else if (loop_state == ST_RISE && f_index == ST_RISE) {
    loop_state = f_index = ST_HIGH;
  } else if (loop_state == ST_FALL && f_index == ST_FALL) {
    loop_state = f_index = ST_LOW;
  }
  return rcode;
}
...
static int replay_file(tcpreplay_t *ctx, int idx) {
  ...
  if (!ctx->options->preload_pcap) {
    ...
    if (pcap_snapshot(pcap) < 65535)
      warnx("%s was captured using a snaplen of %d bytes.  This may mean you "
            "have truncated packets.",
 ...
 */
  ...
  if (pcap != NULL) {
    ...
    if (ctx->intf1dlt != ctx->options->file_cache[idx].dlt)
      tcpreplay_setwarn(ctx, "%s DLT (%s) does not match that of the "
                             "outbound interface: %s (%s)",
                        path, pcap_datalink_val_to_name(pcap_datalink(pcap)),
                        ctx->intf1->device,
...
static int replay_two_files(tcpreplay_t *ctx, int idx1, int idx2) {
  ...
  if (pcap1 != NULL) {
    ...
    if (pcap_snapshot(pcap1) < 65535) {
      tcpreplay_setwarn(ctx, "%s was captured using a snaplen of %d bytes.  "
                             "This may mean you have truncated packets.",
    ...
    if ((ctx->intf1dlt >= 0) && (ctx->intf1dlt != pcap_datalink(pcap1))) {
      tcpreplay_setwarn(
          ctx, "%s DLT (%s) does not match that of the outbound interface: "
               "%s (%s)",
    ...
    if (ctx->intf1dlt != ctx->intf2dlt) {
      tcpreplay_seterr(ctx, "DLT mismatch for %s (%d) and %s (%d)", path1,
                       ctx->intf1dlt, path2, ctx->intf2dlt);
      return -1;
    }
  }
