...
tcpreplay_init()
{
    ...
    ctx = tcpr_safe_malloc(sizeof(tcpreplay_t));
    ctx->options = tcpr_safe_malloc(sizeof(tcpreplay_opt_t));
    ...
    ctx->options->tcpdump = (tcpdump_t *)tcpr_safe_malloc(sizeof(tcpdump_t));
#endif
...
tcpreplay_post_args(tcpreplay_t *ctx, int argc)
{
    ...
    ebuf = tcpr_safe_malloc(SENDPACKET_ERRBUF_SIZE);
#ifdef DEBUG
    if (HAVE_OPT(DBUG))
