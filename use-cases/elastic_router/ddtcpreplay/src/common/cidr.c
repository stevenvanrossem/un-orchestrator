...
ip2cidr(const unsigned long ip, const int masklen)
{
    ...
    network = (u_char *)tcpr_safe_malloc(20);
...
new_cidr(void)
{
    ...
    newcidr = (tcpr_cidr_t *)tcpr_safe_malloc(sizeof(tcpr_cidr_t));
...
new_cidr_map(void)
{
    ...
    new = (tcpr_cidrmap_t *)tcpr_safe_malloc(sizeof(tcpr_cidrmap_t));
...
cidr2iplist(tcpr_cidr_t * cidr, char delim)
{
    ...
    list = (char *)tcpr_safe_malloc(size);
