...
#ifdef ENABLE_UNIFY_MONITORING_CONTROLLER
		/**
		*	8) Starts the monitoring controller related to this graph
		*/
...
bool GraphManager::updateGraph(string graphID, highlevel::Graph *newGraph)
{
	...
	*	Stop the monitoring controller
	*/
	logger(ORCH_DEBUG_INFO, MODULE_NAME, __FILE__, __LINE__, "Stopping the monitoring controller before the graph update...");
	MonitoringController *monitoringController = (tenantLSIs.find(graphID))->second.getMonitoringController();
	monitoringController->stopMonitoring();
#endif
	...
	if(!updateGraph_add_rules(graphID,diff_to_add))
	{
		...
		*/
		logger(ORCH_DEBUG_INFO, MODULE_NAME, __FILE__, __LINE__, "Starting the monitoring controller after the graph update...");
		highlevel::Graph *graph = (tenantLSIs.find(graphID))->second.getGraph();
		LSI *lsi = (tenantLSIs.find(graphID))->second.getLSI();
...
highlevel::Graph *GraphManager::updateGraph_add(string graphID, highlevel::Graph *newGraph)
{
	...
	*/
	...
	graph->setMeasureString(diff->getMeasureString());
#endif
