...
#ifdef ENABLE_UNIFY_MONITORING_CONTROLLER
	//IF the monitoring string is changed, add it to the diff
	string originalMeasureString = this->getMeasureString();
	string otherMeasureString = other->getMeasureString();
	if(originalMeasureString != otherMeasureString)
		diff->setMeasureString(otherMeasureString);
#endif
